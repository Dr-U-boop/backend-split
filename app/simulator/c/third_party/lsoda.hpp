...
namespace LSODA {
  ...
  public:
    ...
    */
    ...
    {
      if (!(tout > *t)) throw std::runtime_error("tout <= *t");
	...
	std::cerr << "[lsoda] illegal istate = " << *istate << "\n";
	terminate(istate);
	return;
      }
      if(itask < 1 || itask > 5) {
	std::cerr << "[lsoda] illegal itask =" << itask << "\n";
	terminate(istate);
	return;
      }
	...
	ntrep = 0;
	if(neq <= 0) {
	  std::cerr << "[lsoda] neq = " << neq << " is less than 1." << "\n";
	  terminate(istate);
	  return;
	}
	if(*istate == 3 && neq > n) {
	  std::cerr << "[lsoda] istate = 3 and neq increased" << "\n";
	  terminate(istate);
	  return;
	}
	n = neq;
	if(itol_ < 1 || itol_ > 4) {
	  std::cerr << "[lsoda] itol = " << itol_ << " illegal" << "\n";
	  terminate(istate);
	  return;
	}
	if(iopt < 0 || iopt > 1) {
	  std::cerr << "[lsoda] iopt = " << iopt << " illegal" << "\n";
	  terminate(istate);
	  return;
	}
	if(jt == 3 || jt < 1 || jt > 5) {
	  std::cerr << "[lsoda] jt = " << jt << " illegal" << "\n";
	  terminate(istate);
	  return;
	}
	jtyp = jt;
	if(jt > 2) {
	  ...
	  mu = iworks[1];
	  if(ml >= n) {
	    std::cerr << "[lsoda] ml = " << ml << " not between 1 and neq" << "\n";
	    terminate(istate);
	    return;
	  }
	  if(mu >= n) {
	    std::cerr << "[lsoda] mu = " << mu << " not between 1 and neq" << "\n";
	    terminate(istate);
	    return;
	  }
	}
	...
	/* Optional inputs.   */
	else /* if ( iopt = 1 )  */
	  {
	    ixpr = iworks[2];
	    if(ixpr > 1) {
	      std::cerr << "[lsoda] ixpr = " << ixpr << " is illegal" << "\n";
	      terminate(istate);
	      return;
	    }
	...
	*/
	...
	/* Initial call to f.  */
	if(!((int)yh_.size() == lenyh + 1)) throw std::runtime_error("(int)yh_.size() != lenyh + 1");
	if(!(yh_[0].size() == nyh + 1)) throw std::runtime_error("yh_[0].size() != nyh + 1");
	...
	*/
	...
	if(kflag == -1 || kflag == -2) {
	  ...
	  if(kflag == -1) {
	    ...
	    fprintf(stderr, "         with std::abs(h_) = hmin\n");
	    *istate = -4;
	  }
	  if(kflag == -2) {
    ...
    {
      if(!(neq + 1 == y.size())) throw std::runtime_error("neq + 1 != y.size()");
